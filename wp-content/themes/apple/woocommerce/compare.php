	...
	<table class="compare-list" cellpadding="0" cellspacing="0" <?php if ( empty( $products ) ) {
		...
		<?php else : ?>
			...
			foreach ( $fields as $field => $name ) :
				...
				foreach($products as $product_id => $product ) {
					if(
						empty($product->fields[$field]) ||
						($product->fields[$field] == '<span>N/A</span>') ||
						($product->fields[$field] == '<span>-</span>')
					) {
						$count_empty_attrs++;
					}

					var_dump($count_empty_attrs);

					if($count_empty_attrs == $total_products) {
						continue;
					}
				}
				?>
					...
					foreach ( $products as $product_id => $product ) :
						...
						<td class="<?php echo esc_attr( $product_class ); ?>">
							...
							switch ( $field ) {
								...
								case 'image':
									echo '<div class="image-wrap" style="">' . $product->get_image( 'yith-woocompare-image' ) . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
									break;
